...
module extras(){
    ...
    /**/
    %translate([-18.52,-0+5-31/2-offset,41.6-(5-plate_thickness)])rotate([0,90,0])nema(nema_height=3,shaft_dia=8);
    %color("red")translate([-18.5,-31.45,plate_thickness])import("extruder_corner_shaved.stl");
    %color("red")translate([-18.5,21.5,plate_thickness])import("extruder_corner.stl");
    ...
    color("RoyalBlue")difference(){
        union(){
            linear_extrude(plate_thickness)
               polygon( points=[[-45,-y],[-60,-15],[-60,10],[-43,y2],[17,y2],[30,y2-15],[30,-y+10],[20,-y]]);
            minkowski(){
...
difference(){
    ...
    translate([-18.52,-0+5-31/2-offset,41.6-(5-plate_thickness)])rotate([0,90,0])nema(nema_height=10);
    //bolt holes
...
if (print == 1){
    translate([0,0,plate_thickness])rotate([180,0,0]) 
        baseplate();
    translate([-10,35,-x+upright_thickness])rotate([0,90,90]) 
        nema_mount();
    translate([-8,56,0.25]) 
        filament_inlet();
}else {
